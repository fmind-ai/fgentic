...
func TestClientContract_MessageContextAttributionAndWireVersion(t *testing.T) {
	...
	second, err := client.Call(ctx, contractTarget(t), "second prompt", first.ContextID, nil)
	if err != nil {
...
func TestClientContract_TerminalTask(t *testing.T) {
	...
	result, err := client.Call(t.Context(), contractTarget(t), "run task", "", nil)
	if err != nil {
		t.Fatalf("Call: %v", err)
	}
...
func TestClientContract_WorkingTaskCanBePolledToCompletion(t *testing.T) {
	...
	working, err := client.Call(t.Context(), contractTarget(t), "long task", "", nil)
	if err != nil {
		t.Fatalf("Call: %v", err)
	}
...
func TestClientContract_ExecutorErrorIsContextualized(t *testing.T) {
	...
	_, err := client.Call(t.Context(), contractTarget(t), "fail", "", nil)
	if err == nil {
...
func TestClientContract_EmptyMessageCrossesWire(t *testing.T) {
	...
	result, err := client.Call(t.Context(), contractTarget(t), "return no content", "", nil)
	if err != nil {
		t.Fatalf("Call: %v", err)
	}
...
func TestClientContract_CallHonorsDeadline(t *testing.T) {
	...
	go func() {
		_, err := client.Call(ctx, contractTarget(t), "wait forever", "", nil)
		callDone <- err
	}()
