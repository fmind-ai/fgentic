...
1. It maps the ghost (`agent-platform-helper`) to either a local kagent `(namespace, name)` or a pinned remote A2A `url` via `agents.yaml` — the **authorization allowlist**: only mapped ghosts are invokable, each mapping binds exact `allowedRooms`, only allowed senders/homeservers may invoke them (federated look-alikes are rejected), and per-sender/per-room token buckets guard LLM spend. Before every initial or resumed A2A action, the event room must match that binding and the ghost must already be joined. No message path joins a ghost. Each local mapping declares its highest `dataClassification`; omission defaults to `regulated`, and the bridge carries the value only on its authenticated local agentgateway request. Remote targets add a request timeout, partner-enforced token-budget metadata, and expected signed-card identity; they never receive the local classification header. Configured `bridgedOrigins` classify only anchored full-MXID namespaces; Slack, Telegram, and future bridged senders are denied until the target agent's `allowedSenders` explicitly matches them. Terminal audits expose only bounded `sender_origin_kind`/`sender_origin_network` attribution in addition to the existing Matrix identifiers.
...
1. An authorized invoker or room moderator may run `!forget <agent>` for a local mapping. The bridge refuses while that room/agent has durable work, deletes every observed kagent session owner, verifies each session is no longer readable, and only then resets the context. `maxSessionAge` applies the same verified reset automatically per local room/agent in bounded sweeps. Remote mappings fail closed because A2A does not define a trusted deletion operation.
1. When the bot first accepts a room invitation, it posts one standalone onboarding `m.notice` containing that inviter's sender-filtered gallery plus the mention, directory, plaintext-command, and budget entry points. A permanent content-free room marker and a room-derived Matrix transaction ID prevent replay on rejoin or restart. The shared notice plane may silently suppress the one attempt; `WELCOME_ENABLED=false` disables it.
...
1. State (mautrix StateStore, versioned transaction/delegation ledger, contexts, permanent room-welcome markers, and a legacy processed-event compatibility table) persists in Postgres (`DATABASE_URL`); without it the bridge runs dev-only in-memory state. Terminal transitions scrub event/prompt/result content, retain delivered/denied tombstones for at least 24 hours, and preserve ambiguous/dead evidence for operator review.
1. Durable recovery covers initial mentions, known-task polling, authorized room-reaction cancellation, paused input-required continuation by the original full MXID, bounded threaded progress, optional pin/unpin convergence, and Matrix reply/placeholder/edit projection. A2A send/cancel/continuation remains at-most-once at an unknown acknowledgement; deterministic Matrix controls replay or reconcile under fenced ledger state. Typing remains process-local and is not crash-safe evidence.
...
mise run test:crash-recovery # twelve persisted SIGKILL boundaries in disposable kind
...
| `FEDIVERSE_BROKER_URL`                              | _(empty)_                                                              | Private ActivityPub gateway side-port URL; configure with the broker token for `acct:` mappings.    |
| `FEDIVERSE_BROKER_TOKEN`                            | _(empty)_                                                              | Dedicated broker bearer; never forwarded to a remote organization.                                  |
| `KAGENT_API_URL`                                    | `http://kagent-controller.kagent.svc.cluster.local:8083`               | Internal controller origin for verified local-session deletion.                                     |
| `CONVERSATION_SWEEP_INTERVAL`                       | `1h`                                                                   | Maximum delay before an expired local `maxSessionAge` policy is swept.                              |
| `GHOST_PREFIX`                                      | `agent-`                                                               | Local-part prefix for agent ghosts.                                                                 |
| `ACCESS_MANAGER_MXID`                               | `@alice:<SERVER_NAME>`                                                 | Exact local MXID permitted to invite mapped ghosts into their declared rooms.                       |
...
| `WELCOME_ENABLED`                                   | `true`                                                                 | Post one sender-filtered onboarding notice when the bot first accepts a room invitation.            |
...
| `INPUT_WAIT_TIMEOUT`                                | `10m`                                                                  | Separate bounded wait for the original sender's threaded answer; pauses the task deadline.          |
...
| `CONTROL_CAPACITY_PER_JOB`                          | `16`                                                                   | Max durable controls retained per delegation, including one terminal-unpin reserve.                 |
...
| `DELEGATION_MAX_ATTEMPTS`                           | `5`                                                                    | Failures before terminal `dead`; post-delivery dead-man cleanup keeps retrying.                     |
| `CANCEL_MODERATOR_POWER_LEVEL`                      | `50`                                                                   | Power needed to cancel another sender's task; the original sender is always allowed.                |
| `MAX_TASK_PROGRESS_POSTS`                           | `3`                                                                    | Max durable threaded progress projections per delegation; `0` disables them.                        |
| `PIN_IN_FLIGHT_TASKS`                               | `false`                                                                | Durably pin/unpin working placeholders when the ghost has pinned-state power.                       |
...
## Conversation forgetting and erasure limits

`!forget <agent>` is a functional conversation-memory reset, not a claim that every copy of the underlying data was physically erased. At pinned kagent v0.9.11, `DELETE /api/sessions/{contextId}` soft-deletes the session for one user; the bridge calls it for every Matrix sender it observed on that room/agent context, requires the follow-up GET to return 404, and then drops its own context so the next delegation receives a new ID. Pre-governance contexts have no complete historical owner inventory. Later turns cannot reconstruct missing identities, so commands and retention remain fail-closed for those rows until an operator cleans up the legacy kagent context and replaces the bridge context as one coordinated action.

The command does **not** redact Matrix room events or media, recall federated copies, remove backups, purge an agent's independent long-term memory/tool stores, or physically delete kagent's soft-deleted database rows and events. A GDPR erasure workflow must therefore pair this bridge control with the deployment's kagent/Postgres deletion or retention procedure, Matrix homeserver/media erasure, backup expiry, and equivalent action by every participating federation server. Those cross-organization limits are the same replication boundary described in [`docs/federation.md` §8.1](../../docs/federation.md).
...
An `acct:agent@org.example` mapping replaces only the fixed `url`. It still requires the same `cardIdentity` for a discovered A2A upgrade and additionally pins `activityPubIdentity.actorID`, `verificationMethod`, `publicKeyMultibase`, and a positive `proofMaxAge`. The private broker resolves WebFinger and the actor on every trust refresh. If `implements` advertises A2A, the bridge verifies the signed card against its exact discovered endpoint and card URL before calling the official SDK. Otherwise the broker verifies the fresh FEP-8b32 actor proof and delivers a signed AP `Create(Note)`. A handle or DNS answer alone never authorizes delegation; the fallback acknowledgement states honestly that any reply is asynchronous.

## Onboarding and discovering agents in a room

Create an unencrypted, private, invite-only, purpose-scoped room at room version 12 or later, with `ACCESS_MANAGER_MXID` as its sole creator. Room v12 creators have implicit unlimited power and must not appear in `m.room.power_levels.users`; keep ordinary users and ghosts at 0, set `invite`, `kick`, `ban`, `redact`, and `state_default` to 100, and ordinary `m.room.message` events to 0. Matrix defaults `invite` to 0, so an invite-only join rule without these power levels does not establish the access-manager boundary. Put the exact room ID in each intended agent's `allowedRooms`; a local room alias is also accepted as a bootstrap locator and is resolved again on every admission, while a direct room ID avoids alias indirection.

Invite `@a2a-bridge:<server-name>`. On its first accepted invitation, the bot posts one bounded welcome with only the agents that the full inviter MXID could invoke in that room. The welcome is never repeated on rejoin. Only `ACCESS_MANAGER_MXID` may invite a mapped ghost, and only into one of that ghost's declared rooms. The bridge accepts that managed invite; an unauthorized or unbound invite stays unjoined and emits content-free evidence. A mention never joins an absent ghost and fails before rate or A2A admission. Once the access manager has invited the ghost, send:
