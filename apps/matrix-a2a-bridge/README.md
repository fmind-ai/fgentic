...
Human in a Matrix room:  "@agent-platform-helper why is the bridge pod not ready?"
   ...
   bridge:  detect @mention → map @agent-platform-helper → kagent/platform-helper
            ...
            fenced per-room lease → A2A message/send (non-streaming)
   ◀  reply text
