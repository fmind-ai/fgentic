...
Create an unencrypted, purpose-scoped agent room and invite `@a2a-bridge:<server-name>`. On its first accepted invitation, the bot posts one bounded welcome with only the agents that the full inviter MXID may invoke. The welcome is never repeated on rejoin. Invite a mapped ghost when you are ready to delegate, then send:
