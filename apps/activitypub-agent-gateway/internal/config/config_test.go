...
func TestValidateRejectsBadInput(t *testing.T) {
	base := func() Config {
		return Config{
			...
			SignatureFutureSkew:   5 * time.Minute,
	...
	tests := map[string]func(*Config){
		...
		"bad future skew":   func(c *Config) { c.SignatureFutureSkew = 0 },
		"large future skew": func(c *Config) { c.SignatureFutureSkew = 6 * time.Minute },
		"future above max":  func(c *Config) { c.SignatureMaxSkew = time.Minute; c.SignatureFutureSkew = 2 * time.Minute },
