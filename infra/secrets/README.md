...
`knowledge-ingestion.sops.yaml` keeps the DML-only database role copies and the separate agentgateway caller copies coherent for the opt-in batch workload. The database password is identical in `postgres` and `knowledge`; the workload key is identical in `agentgateway-system` and `knowledge`. Neither value is a schema-owner or model/provider credential. The core generator excludes this file: create it explicitly with `FGENTIC_SECRET_SET=knowledge-ingestion` and rotate it by the same named set.

Operational corpus bytes are not secrets configuration and must not be stored in SOPS, plaintext ConfigMaps, or this GitOps/deployment repository. Provision the `knowledge-source-bundle` PVC separately under the deployment's storage, encryption, backup, and access policy; the ingestion profile intentionally creates no PVC or StorageClass. A separately governed source Git repository may feed the #335 connector.
