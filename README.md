...
The client-safe plaintext command fallback works in Matrix clients without ghost-MXID autocomplete: `!agents` lists the agents you may invoke, `!ask <agent> <prompt>` uses the same policy and cost-admission path as an `@mention`, and `!budget` shows the current request limits plus remote per-request token reservation ceilings. Leading-slash forms (`/agents`, `/ask`, `/budget`) also work when the client sends them as messages instead of handling them locally. The budget view is read-only and never presents a reservation as observed consumption. Native Matrix command-picker work remains tracked separately in [#223](https://github.com/fmind-ai/fgentic/issues/223).
