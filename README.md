...
1. **Governed by construction.** Locally hosted agent LLM egress flows through agentgateway (single credential, unified telemetry, token budgets); every invocation is attributable to a Matrix identity. An explicitly pinned remote A2A agent is an external trust boundary whose model egress and enforcement belong to the partner.
