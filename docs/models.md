...
Each local `agents.yaml` mapping declares `dataClassification`; omission defaults to the most restrictive `regulated` class. The bridge injects `X-Fgentic-Data-Classification` only on its local A2A transport, beside its independently authenticated workload key, and never sends that policy metadata to a configured remote target. The common A2A route owns one combined agentgateway policy because pinned agentgateway v1.3.1 selects only one policy for an HTTPRoute target. It is reconciled with that route before the later provider inventory, so provider failure or pruning cannot remove workload authentication. The policy admits a `SendMessage` only when the exact substituted provider/model exists in the catalog and its `allowedClassification` ceiling covers the mapping's class. Missing and unknown headers fail closed.
