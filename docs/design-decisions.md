...
### D9 — Long tasks via `GetTask` + Matrix edits (was: hard 60s ceiling)

Real agent work exceeds 60s; kagent deliberately sets no timeout on its agent client and proxies `GetTask`. **Now:** §6's model — non-terminal `Task` → placeholder reply → poll with backoff up to `TASK_TIMEOUT` (10m) → `m.replace`-edit the placeholder into the final answer. The whole-task clock begins at the first persisted A2A attempt boundary rather than ledger admission, so durable room backlog is excluded. Matrix edits are the deliberate open-standard substitute for streaming; `SendMessage` stays non-streaming.
...
**Review (2026-07-24): not adopted.** The repository remains deliberately pinned to v0.9.11, while the newest non-prerelease upstream release is [v0.9.12](https://github.com/kagent-dev/kagent/releases/tag/v0.9.12). That release contains only the declarative-agent tag backport, and its load-bearing authentication files are unchanged from v0.9.11: `trusted-proxy` still [decodes the JWT payload without validating its signature](https://github.com/kagent-dev/kagent/blob/v0.9.12/go/core/internal/httpserver/auth/proxy_authn.go#L28-L78), relies on oauth2-proxy as the authentication boundary, and the controller still installs the [no-op authorizer](https://github.com/kagent-dev/kagent/blob/v0.9.12/go/core/cmd/controller/main.go#L32-L47). That browser-oriented proxy flow cannot authenticate Fgentic's non-browser Matrix bridge: Matrix events carry a sender ID, not the sender's OIDC bearer token, so the bridge can only assert that sender through `X-User-Id`. Upstream's [non-browser authorization hook #1890](https://github.com/kagent-dev/kagent/issues/1890) was closed as not planned without delivering the hook, while kagent's per-Agent/API authorization remains open in [#1270](https://github.com/kagent-dev/kagent/issues/1270). Re-evaluate only after #1270 (or its accepted successor) ships in a non-prerelease release **and** a stable non-browser authorization hook is configured end to end. Until both conditions hold, `X-User-Id` is attribution rather than authentication and the NetworkPolicy conformance guard remains mandatory.
