...
1. `awaiting_input` has an acknowledged task/context pair, a separately bounded human-response deadline, and a paused task deadline. Join its `bridge_delegation_controls` rows by `job_id`: `prepared` cancel/continuation means the A2A side effect may have started and recovery must not resend; question/progress/pin projections may replay only with their stored `matrix_txn_id`.
...
Inspect controls without selecting their active `payload`:
...
`recovery_count > 0` on cancel/continuation is the persisted no-resend ambiguity signal. Terminal controls have an empty payload; the retained 32-byte intake fingerprint detects changed evidence without retaining the answer, question, or progress text.
...
| `canceled_by`                              | Full MXID of the room member who canceled a task. Durable cancellation copies it from the authorized, pre-acknowledgement control record and terminally edits the placeholder with the same identity.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `secret_scan_action`                       | Reply→room secret-scan enforcement applied to this reply (#343): `annotate`, `redact`, or `block`; empty when the scan was off or found nothing. Content-free — it names the mode, never a matched value.                                                                                                                                                                                                                                                                                                                                                                                                              |
| `secret_match_count`/`secret_rule_classes` | Bounded, content-free evidence of a reply-scan hit: the number of masked spans and the sorted set of fixed rule classes (e.g. `aws-access-key-id`, `private-key`) that matched. Never the matched value, span, or reply text; both empty/zero on a clean or off reply.                                                                                                                                                                                                                                                                                                                                                 |
...
For a reply-secret block (#343), require `outcome=denied`, `terminal_stage=message_result` or `task_result`, `terminal_reason=secret_in_reply`, `a2a_attempted=true` (the scan runs after the A2A call), and `secret_scan_action=block` with a positive `secret_match_count`; require one increment of `fgentic_reply_secret_scans_total{action="block"}`. A masked-but-delivered reply keeps `outcome=ok`/`terminal_reason=completed` and carries `secret_scan_action=annotate|redact` with the same content-free count/classes. In every case the matched value is absent from the audit, the metric, the log, and the Matrix event — join none of these fields to a secret value; Matrix (with the reply masked or withheld) remains the only record.
...
| Bridge durable ledger        | Active jobs retain the recoverable Matrix event/prompt and later a pending result/notice; `awaiting_input` briefly retains its question in the job result field until the deterministic question control is planned, and active controls may retain a question, answer, or progress body. Each terminal job/control transition clears its content columns while retaining content-free fingerprints and protocol IDs. Non-content `delivered`/`denied` tombstones and unreferenced transaction hashes become eligible for periodic cleanup after at least 24 hours; `ambiguous`/`dead` evidence remains indefinitely. Legacy `bridge_processed_events` markers retain their own minimum 24-hour compatibility window. | Restrict the bridge database and its backups as content-bearing while any job or control is active. Treat retained event/room/sender/A2A/Matrix IDs as linkable operational data. Do not delete ambiguous/dead evidence before investigation; changing these code-level windows requires privacy, recovery, and replay review. |
...
| kagent sessions and tasks    | Persistent Postgres rows. Current agent mappings omit the optional `maxSessionAge`, so conversations remain until an authorized local `!forget`/`/forget` reset by default. That control and configured age sweeps delete every recorded owner's kagent session, verify it is unreadable, and only then reset the bridge context. Remote mappings and incomplete pre-governance owner inventories fail closed.                                                                                                                                                                                                                                                                                                        | Set `maxSessionAge` on a reviewed local mapping or invoke `!forget`; do not use ad-hoc SQL. This is functional reset, not physical deletion of kagent's soft-deleted rows/events, independent stores, or backups. CloudNativePG backup catalog retention is `30d`, with a GCS 60-day lifecycle backstop.                       |
...
1. **Recovery fixtures are not an RTO.** Unit/contract tests prove state-machine behavior, `test:availability` exercises graceful SIGTERM, and `test:crash-recovery` SIGKILLs the bridge at twelve persisted job/control boundaries. Even a green local fixture is not node-loss evidence or a production recovery-time objective.
...
1. **Admin-action audit attributes an admin-API call, not intent, and only for URL-determined mutations.** The opt-in `fgentic.admin_action.v1` stream ([ADR 0018 — Admin-action audit](adr/0018-content-bounded-identity-audit.md#admin-action-audit-455)) projects the pinned Synapse admin request log to record who purged a room, quarantined media, or dismissed a report, with outcome. It deliberately does **not** assert suspend vs reactivate direction (a request-body argument the content-free record excludes), does **not** attribute registration-token or MAS-plane admin actions (no content-free, non-secret source at the pinned versions), and sees **only** admin-API calls — a direct database or `kubectl` change is invisible to it. The gateway access log cannot substitute: it sees an opaque bearer, never the acting MXID.
...
The permanent tests cover the content-free audit schemas, exact-body replay/conflict, fenced job/control transitions, consecutive retry/dead behavior, acknowledgement ambiguity without resend, known-task and paused-input recovery, bounded progress, deterministic Matrix projection and pin convergence, terminal content scrubbing/retention, explicit rate-limit verdict, A2A header/context contract, collector joins, and absent/ambiguous records. The crash-recovery task adds twelve process-kill boundaries against real Postgres, Synapse, and A2A; retain its candidate-revision result, because the fixture is not part of a green unit suite by implication. After deployment, complete one fresh mention and run the collector above. That live walk—not local test evidence alone—is the acceptance proof for the installed versions.
