...
1. D17 supersedes the process-local queue and event-marker completion portions of D3 and D4 while preserving D7's bounded `queue_full` contract in durable admission. The 32-per-room/256-global defaults now count non-terminal ledger jobs rather than process memory; a refusal is terminal evidence rather than an unrecorded drop. D5's `(room, agent)` context key and D9's `tasks/get`/Matrix-edit protocol remain unchanged.
