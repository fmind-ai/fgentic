...
At the time of this decision, the bridge had two controls from D6: `allowedServers` and anchored `allowedSenders` globs over the full MXID. They were useful ceilings, especially for federation, but a poor dynamic group database: they had no room dimension and had to be regenerated when a group changed. The bridge also auto-accepted every valid invite for a mapped ghost and called `EnsureJoined` during dispatch, so the room-membership design was incomplete until each ghost could be restricted to explicitly managed rooms. [PR #693](https://github.com/fmind-ai/fgentic/pull/693) delivered that boundary: the Helm chart still preserves and tests both D6 fields, while a mapped ghost now joins only an exact declared room after an access-manager invite and dispatch never joins it ambiently.
...
1. Maintainer approval is recorded in issue #19. This acceptance settles the design only; D6 and the implementation gates below remain authoritative through rollout.
1. [PR #693](https://github.com/fmind-ai/fgentic/pull/693) closed issue #155 after delivering and federation-proving exact `allowedRooms`, managed-invite, current-ghost-membership, and no-ambient-join hardening. [PR #948](https://github.com/fmind-ai/fgentic/pull/948) delivered issue #154's opt-in, audit-only-by-default reconciler offline core through a scoped Matrix client identity, without Spaces or Synapse/MAS administrator credentials. Issue #154 remains open only for the recorded full local-profile convergence proof against live Keycloak and Matrix; neither merged source nor this ADR claims that runtime acceptance.
1. Bridge hardening remains a prerequisite for every group grant. The delivered no-ambient-join and managed-invite controls, plus the existing Helm preservation tests for `allowedServers`/`allowedSenders`, are continuing regression guards.
1. Roll out in audit-only mode first: validate `matrix_localpart`, compute membership diffs, and compare them with existing rooms without mutation. Any duplicate subject/localpart, missing or invalid localpart, nonexistent Matrix account, unmanaged room, unexpected creator, or power-level drift fails closed for grants. PR #948 ships this audit-only default; live enforcement still requires the remaining issue #154 acceptance.
