...
1. **The authorization key is a typed triple:** complete user principal, exact Agent identity, and memory scope.
   ...
   1. The default memory scope is one explicitly managed private 1:1 room. It never inherits an ambient room, Space, homeserver, or localpart namespace.
...
1. **Consent precedes collection and retrieval.**
   ...
   1. Withdrawal first blocks every later memory operation, then erases the scope. If erasure cannot be verified, the scope remains disabled and reports a content-free operator failure; it never resumes on a best-effort assumption.
...
1. **User controls are exact and private.** The initial Matrix contract is:
   ...
   1. Only the exact subject may inspect or erase personal memory. A room moderator or platform operator cannot retrieve its content through the Matrix UX. Commands execute and reply only in the subject's managed private 1:1 control room; group and federated rooms receive no memory content.
...
1. **kagent remains the content store; the bridge stores control state only.**
   ...
   1. Before implementation, the kagent boundary must support a scope distinct from attribution, per-entry hard deletion with read-after-delete verification, an immutable maximum expiry, and a hard per-scope entry cap. Pinned `v0.9.11` satisfies none of those four requirements as a complete set.
...
1. **Federation is denied by default.**
   1. The first implementation admits only a native local MXID in a local-only private 1:1 room whose ACL excludes every remote server.
...
1. **The first prototype is deliberately narrow and cost-bounded.**
   1. One sample Agent, one native local test user, and one local-only private 1:1 room are enabled on the disposable demo profile only. No remote identity, group room, default profile, or production claim is included.
   ...
   1. The candidate policy uses a strict seven-day maximum, at most 100 entries per scope, bounded entry length, one prefetch per new session, and no background save more often than once per five user turns. The implementation must meter embedding, summarization, save, and retrieval operations separately from chat tokens; reservations and operation counts are not reported as model-token consumption.
...
1. **Implementation work is cut only after human privacy approval.** The approved sequence is expected to be:
   1. upstream or adapter capability: typed scope, per-entry delete, immutable expiry, cap, and per-request disable;
...
1. Pinned kagent needs upstream work or a separately reviewed adapter before the proposal is implementable. That dependency is intentional: whole-store deletion and popularity-extended TTL cannot be presented as exact "forget X" and maximum retention.
...
1. Inspection necessarily returns sensitive content to Matrix. Restricting it to a managed private 1:1 control room reduces exposure but does not make Matrix history erasable; the normal retention, backup, device-cache, and federation caveats still apply.
