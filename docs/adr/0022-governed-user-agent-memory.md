...
1. **kagent remains the content store; the bridge stores control state only.**
   1. Memory text, embeddings, search queries, search results, summaries, and upstream response bodies stay in kagent's scoped PostgreSQL database. They never enter bridge tables, logs, metrics, traces, audit events, exception text, or Matrix room state.
