...
1. The single-replica, per-room-ordering invariant (D3) is not weakened by any accretion; sibling apps must tolerate the bridge's non-overlapping rollout window without data loss (state lives in Postgres or Matrix room state, never in cross-app memory).
