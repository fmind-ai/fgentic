...
| ID   | Vendor-neutral requirement                                                                                                                                           | Evidence to request                                                                                                                                                   | Fgentic reference mapping                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GC-1 | Admission, routing, tool authority, data classification, and consequential-action policy shall be deterministic controls outside the model.                          | Policy source, change approval, deny tests, rollback procedure, and proof that prompt text cannot grant authority.                                                    | **Implemented / Configured / Deferred:** agent mappings, sender policy, admission policy, gateway authorization, the read-only MCP catalog, and NetworkPolicy provide bounded controls. Local mappings send a reviewed classification to the authenticated gateway, and the model catalog declares routing ceilings; neither classifies content nor proves compliance approval. General classification enforcement, consequential-action policy, and universal human approval are not implemented. |
...
| GC-3 | Every deployable artifact and configuration change shall have an immutable identity, review history, vulnerability process, provenance boundary, and rollback route. | Signed artifact and SBOM verification; dependency inventory; vulnerability response route; Git history; deployment reconciliation and rollback evidence.              | **Configured:** the bridge image is built, vulnerability-scanned, SBOM- and provenance-attested, signed, and digest-pinned. The chart is packaged, provenance-attested, signed, and digest-pinned. Flux reconciles reviewed Git; live rejection and upstream provenance remain separate.                                                                                                                                                                                                           |
...
| ID   | Vendor-neutral requirement                                                                                                                                              | Evidence to request                                                                                                                                          | Fgentic reference mapping                                                                                                                                                                                                                                                                                                             |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IN-1 | Collaboration, agent delegation, tool access, identity, deployment, and observability interfaces shall use documented, versioned standards with explicit negotiation.   | Exact protocol versions and extensions; conformance results; negative version tests; compatibility policy; no proprietary service required in the path.      | **Implemented / Configured:** Matrix, A2A, MCP, OIDC, Kubernetes, Gateway API, PostgreSQL, Git, and OpenTelemetry form the seams. Repository-controlled implementations and extensions are pinned or documented where supported; the GKE control plane follows its release channel, and target negotiation evidence remains external. |
...
flowchart TB
  ...
  accDescr: Element and Matrix handle collaboration and identity, the bridge turns mentions into local or pinned-remote A2A work, authorized partners use a separate public gateway route, and PostgreSQL and observability provide state and evidence.

  User["Element or Matrix client"] -->|Matrix client API| Matrix["Synapse + MAS<br/>collaboration and identity"]
  PartnerHS["Partner Matrix homeserver"] <-->|Matrix federation| Matrix
  Matrix -->|authenticated appservice transaction| Bridge["Matrix-A2A bridge<br/>policy, durable intake, attribution"]
  Bridge -->|authorized A2A| Gateway["agentgateway<br/>routing, auth, quotas, telemetry"]
  Bridge -->|verified card + remote A2A| RemoteAgent["Pinned remote A2A agent"]
  PartnerClient["Authorized partner A2A client"] -->|JWT + exact public route| Gateway
...
Element positions [ESS Community](https://element.io/server-suite/community) for non-commercial, non-professional community use and evaluation, with a stated small-to-mid-sized range of 1–100 users and no performance guarantees or vendor-backed production support. Its [edition comparison](https://github.com/element-hq/ess-helm#editions) places in-cluster HA, dynamic scaling, advanced security services, and professional support in ESS Pro.

That is an upstream product, support, and sizing posture, not proof that an arbitrary 100-user workload fits and not a commercial-use restriction added by the AGPL license. Open-source Synapse can be engineered with workers and external services, but that is a different, adopter-owned topology unless the selected distribution and acceptance plan deliver it.
