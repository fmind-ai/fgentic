...
Each acquisition and ingestion local-file handoff opens only a regular object and verifies that the same bounded object remains unchanged through the complete read. Replacement, disappearance, size drift, or a special-file target fails before its bytes reach semantic parsing or publication. Kubernetes projected inputs may follow their standard symlink-to-regular-file layout; writable PVC materialization directories and named blob, inventory, and lock paths must instead be real directories or regular files, never symlinks or special files. Special paths are rejected without waiting on an external writer.
