...
1. `knowledge_connector` is introduced only by `components/knowledge-ingestion`. It can publish and inspect connector snapshots, inventories, and desired/applied source state through the versioned schema's narrow tables and functions. It has no privilege on chunks, embeddings, embedding checkpoints, retrieval functions, schema objects, or another database. Only the ingestion Pod's inventory-publication phase receives this credential; the acquisition Pod has no database credential.
