...
The client validates HTTP response framing before it decodes semantic JSON. A response may carry exactly one decimal `Content-Length` or exactly one `Transfer-Encoding: chunked`, never both; optional whitespace around either value is limited to HTTP space and horizontal tab. Duplicate, malformed, unsupported, oversized, and incomplete framing fails closed. Tokenizer responses are capped at 256 KiB and embedding responses at 2 MiB whether the body is declared or streamed. The deterministic `test:knowledge-ingestion` suite proves these client-side transport cases offline; it does not replace live agentgateway reachability or NetworkPolicy acceptance.
