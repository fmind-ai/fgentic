...
Exactly one implementation exists: `git-markdown`. It reads the cluster's existing `flux-system/flux-system` `GitRepository`; there is no registration API, dynamic loading, marketplace, per-repository credential, or arbitrary URL input. A current merge becomes eligible only after source-controller reports exactly one `Ready` condition whose status is the exact string `"True"` and publishes one status artifact with bounded revision, digest, URL, and size. `metadata.generation`, `status.observedGeneration`, and that condition's `observedGeneration` must be positive signed 64-bit integers with the same value; missing, stale, boolean, floating-point, or out-of-range values fail before artifact download. The acquisition Pod has a short-lived projected service-account token and RBAC for one exact `get`. Its default-deny policy permits only DNS, the exact source-controller Pods, and the one environment-substituted Kubernetes API endpoint `/32` and port observed after Service DNAT; it cannot reach PostgreSQL, agentgateway, a model, or a provider.
...
Projected acquisition metadata, credentials, and bounded ingestion inputs may follow the standard Kubernetes symlink-to-regular-file layout. Their readers require the opened descriptor to be a size-bounded regular file whose identity, size, and modification time remain stable through that read; they make no post-open pathname-stability claim. Writable PVC materialization is stricter: its directories and named inputs must be real directories and regular files, never symlinks or special files, and each name must still identify the same unchanged file after the read. Immutable publication reads reject symlinks and special files and require the bytes read to match the opened file's bounded size; the acquisition lock similarly requires its name and opened descriptor to identify the same regular file when acquired. Special-file paths are opened non-blocking so rejection cannot wait for an external writer.
