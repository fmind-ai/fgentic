...
readonly WORK_DIR="$(mktemp -d -t fgentic-production-ha.XXXXXX)"
...
readonly TRAEFIK_VERSION="$(yq -er 'select(.kind == "HelmRelease" and .metadata.name == "traefik") |
  .spec.chart.spec.version' "${GCP_RENDER}")"
readonly ESS_VERSION="$(yq -er 'select(.kind == "OCIRepository" and
  .metadata.name == "ess-matrix-stack") | .spec.ref.tag' "${GCP_RENDER}")"
readonly AGENTGATEWAY_VERSION="$(yq -er 'select(.kind == "OCIRepository" and
  .metadata.name == "agentgateway") | .spec.ref.tag' "${GCP_RENDER}")"
readonly KAGENT_VERSION="$(yq -er 'select(.kind == "HelmRelease" and .metadata.name == "kagent") |
  .spec.chart.spec.version' "${GCP_RENDER}")"
readonly KEYCLOAK_VERSION="$(yq -er 'select(.kind == "HelmRelease" and .metadata.name == "keycloak") |
  .spec.chart.spec.version' "${GCP_RENDER}")"
