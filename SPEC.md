...
>
> **Status (2026-07-10, evening).** **Live end-to-end on the local reference cluster** (`fgentic.localhost`, k3d): @mention → bridge → agentgateway (Vertex AI Gemini via ambient/ADC auth, no agent-held keys) → kagent → in-room ghost reply, with session threading, invite auto-join, sanitized failure replies, TLS from a local CA, per-cluster SOPS secrets, and the §9 metrics stack (bridge delegation metrics + gateway GenAI token metering + LLM spend alert) — everything reconciled by Flux from this repo. GCP: tfstate bucket + APIs applied (remote state live); the GKE cluster + DNS zone apply is gated on explicit approval. §12 defines the validation ladder, §13 the roadmap.
>
> **Strategy update (2026-07-11).** Positioning is now **sovereignty-first**: the platform leads as _the sovereign, out-of-the-box agent platform on open protocols_ (self-hostable, no lock-in at any layer, every component swappable), with **federation as the differentiating destination**, sequenced after the sovereignty milestones. Two consequences are binding: the LLM backend becomes a per-cluster, multi-provider choice through agentgateway (D16), and the executable roadmap moved to **GitHub milestones M0–M11** with per-milestone epic trackers (§13) — issues labeled `agent-ready` are groomed for autonomous coding agents; `needs-human` marks decisions, approvals, accounts, or spend.
>
...
| Observability | kube-prometheus-stack 87.12.5   | Metrics half of §9 **built + verified live**: bridge `fgentic_*` metrics, agentgateway `agentgateway_gen_ai_*` token metering, CNPG PodMonitor, LLM spend alert, Grafana behind the shared Gateway                      | Traces (OTel + Jaeger, bridge spans) are the next §9 increment.                                                                                                                                                                                                                                               |
...
## 9. Observability Spec (metrics half built + verified live in `infra/observability/`; traces are the next increment — §9.2)
