...
1. **Default — a `profiles/{disabled,<variant>}` pair.** Use when a whole sub-DAG is present or absent as a unit (e.g. `infra/knowledge/profiles/`, `infra/admin/profiles/`). The `disabled/` profile renders **zero** objects using the repo's render-nothing idiom — a `placeholder.yaml` ConfigMap plus a `patches:` block that `$patch: delete`s it, so `prune` leaves no footprint; copy `infra/admin/profiles/disabled` verbatim. The `<variant>/` profile lists the real resources. Add ONE Flux Kustomization in `clusters/base/` whose `spec.path` ends at the default `.../profiles/disabled`, then select a variant by keying on a `platform-settings` value in [`clusters/base/provider-selection`](../../../clusters/base/provider-selection/kustomization.yaml) — a Kustomize `Component` whose `replacements` block swaps only the final path segment (by `/` delimiter + index); keeping the path prefix makes an unknown/unset value fail closed. When the base layer itself stays and only a sub-concern toggles, the `disabled` profile instead re-lists the base (`infra/gateway/profiles/disabled` → `resources: [../../base]`). `infra/agentgateway/providers/profiles/<provider>` uses the same mechanism for chat-model selection. (An overlay may also patch `spec.path` directly for a documented cluster-specific switch.)
