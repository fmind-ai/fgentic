...
1. SOPS is not operational corpus storage. Keep source bytes out of this GitOps/deployment repository and provision the operator-owned `knowledge-source-bundle` PVC separately under the deployment's storage policy. The shipped #335 reference connector consumes only this repository's existing reconciled Flux source; a separate non-public source requires a future reviewed connector implementation.
...
1. `knowledge-ingestion.sops.yaml` keeps each separate `pg-knowledge-connector` and DML-only `pg-knowledge-ingestion` pair identical between `postgres` and `knowledge`, and independently keeps the gateway caller key identical between `agentgateway-system/knowledge-ingestion-callers` and the `Bearer` value in `knowledge/knowledge-ingestion-credential`. It contains neither the schema-owner password nor a model/provider credential.
