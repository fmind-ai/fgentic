...
1. **Promote deliberately** — after the local/CI golden gate and staging-room acceptance pass, change only the generated mapping's `stage: dev` to `stage: prod` in a reviewed PR. kagent serves the Agent over A2A at `…/api/a2a/kagent/<name>` with an AgentCard; the bridge ghost is `@agent-<name>:<server_name>`.
