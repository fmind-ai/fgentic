...
The proof verifies the ES256/JCS AgentCard and OIDC discovery contract; rejects missing, malformed, wrong-audience, and unauthorized tokens; validates the A2A method, extension, and bounded integer `maxTokens`; accepts one org-B request; observes aggregate downstream token growth; and rejects a second maximum reservation with HTTP 429. The accepted terminal Task carries one task-bound seller-signed receipt in its A2A metadata; a direct Message cannot prove a terminal task state and fails closed. Org B verifies its independent pinned JWK, exact request hash, task/context/outcome binding, nullable `tokensConsumed`, and tamper rejection; unauthorized and quota-denied requests must not grow the single-writer persistent JSONL archive. Reservation is admission accounting per verified `azp`, not measured consumption. The signer runs after gateway authentication/authorization and never receives a model credential or accepts prompt-driven signing decisions. The Matrix half still requires a room-v12 A/B exchange under an exact server ACL, a separate explicitly local-only room, rejected join plus signed-federation-send attempts from C, and a final disallowed custom event that B retains while A's callback drops and logs it without content. The command leaves the cluster running for inspection.
...
| `all`                    | Automatable core and baseline knowledge rows                    | Excludes optional ingestion/networks, Keycloak client, bootstrap admin, and demo users; rotate those explicitly. |
...
For `all`, reconcile once, wait for the five legacy CNPG roles and the two baseline knowledge `DatabaseRole` resources, restart Keycloak, Synapse, MAS, and kagent, wait for the A2A/MCP policies and provider backend, then restart the bridge last. Complete a fresh SSO login, one platform-helper tool call, its MCP audit record, a knowledge-role login check, and an `@mention` round trip before revoking the old provider key. Rotate the optional ingestion set separately when that layer is composed.
...
1. **Review the narrow contract** — replace the generic description, system message, skill, and golden task with the intended behavior while retaining the shared `agent-zoo-runtime` ServiceAccount, prompt-injection boundary, three content-free tracing switches, explicit full-MXID sender allowlist, and `stage: dev`. Add only reviewed tools through the governed MCP catalog; never place a model credential in an Agent. Iterate with `mise run agent:test <name>` — the fast, offline, zero-spend single-agent pre-PR loop (issue #372): it runs only that agent's `evals/<name>/golden.json` scenarios against the deterministic loopback model through the exact same code path as CI's `test:agents-golden`, so a local pass predicts the CI pass and a failure surfaces the scenario diff without a cluster, Matrix, paid model, or network egress. Then run `mise run check:agent-new`, `mise run check:agents`, and `mise run test:agents-golden` before committing. The structural gate rejects orphan mappings, wildcard or widened senders, missing skills, privileged ServiceAccounts, invalid stages, and missing fixtures; the golden gate executes every checked-in task against the zero-spend loopback model and pins the effective Agent source contract. When the spec or an imported prompt changes, update both the evaluation fixture and mapping `agentContractSHA256`; `check:agents` rejects a split contract. The bridge combines that pin with the live mapping into the content-free `agent_version` recorded for every attempted delegation.
1. **Promote deliberately** — once the shared golden CI gate and staging-room acceptance pass, change only the generated mapping's `stage: dev` to `stage: prod` in a reviewed PR. kagent serves the Agent over A2A at `…/api/a2a/kagent/<name>` with an AgentCard; the bridge ghost is `@agent-<name>:<server_name>`.
1. **Roll back as one reviewed contract** — revert the Agent CRD/prompt and its mapping/digest together, then let Flux reconcile. A mapping-only revert is adopted by the bridge reload loop without a pod restart. Verify the restored `agent_version` with a fresh content-free attribution bundle; never infer rollback from a pod restart or commit message alone.

**Authoring dev loop (offline, zero-spend).** The full inner loop is: `mise run agent:new <name>` (scaffold) → edit the Agent description, system prompt, skill, and `evals/<name>/golden.json` to the intended narrow contract → `mise run agent:test <name>` (run that agent's golden tasks locally; repeat until green) → `mise run check:agent-new` + `mise run check:agents` (structural + contract-pin lint) → open the PR. `agent:test` reuses the same `internal/evaluation` harness and deterministic model as CI, so it needs no k3d cluster, Synapse/MAS/kagent/agentgateway, paid model, or network — a developer iterates entirely on their machine and a local pass predicts CI. Promotion (`stage: dev` → `prod`, #128) and rollback (revert the Agent/prompt + mapping/digest together, #371) are steps 3–4 above; keep the fixture and mapping `agentContractSHA256` in lock-step (`check:agents` rejects a split contract). Do not invent a separate runner — `agent:test` is the single shared path.
...
1. **(Optional) Durable task controls** — set `CONTROL_CAPACITY_PER_JOB` above the configured progress budget (the 16-record default covers three progress posts plus question/answer/cancel/pin/unpin evidence). Repeated inbound events retain at most one authorized and one denied record per kind/input generation. `INPUT_WAIT_TIMEOUT` bounds a paused question independently from `TASK_TIMEOUT`; only the original full MXID may answer its placeholder thread. The original sender may always react ❌ to cancel, while another room member needs `CANCEL_MODERATOR_POWER_LEVEL`. To turn on `PIN_IN_FLIGHT_TASKS`, grant agent ghosts the room's `m.room.pinned_events` state-event power level (raise the ghost's user level or lower the pinned-events event default); without it the durable control records `pin_power_rejected` and delegation work continues. Threaded progress (`MAX_TASK_PROGRESS_POSTS`) needs no extra power.

## Runbook: retire an agent

The mirror of _add an agent_: a deliberate, reviewed decommission. Removing a mapping already fails closed at the bridge, but the ghost, its Matrix account, and its kagent sessions linger unless you sweep them. Audit records are retained, never deleted.

1. **Remove the contract as one reviewed change** — delete the agent's `agents.yaml` mapping and, when repo-owned, its kagent `Agent` CRD (`infra/kagent/agents/<name>/`) and bridge Kustomize Component (`apps/matrix-a2a-bridge/deploy/agents/<name>/`) together, plus their entries in the parent Kustomizations, in one PR. Keep the `evals/<name>/golden.json` fixture out of the same change only if another live agent still references it; otherwise remove it too. Run `mise run check:agents` (it rejects an orphaned Component or a mapping/CRD split) before committing, then let Flux reconcile — the bridge adopts a mapping removal through its reload loop without a pod restart ([docs/bridge.md](../../../docs/bridge.md) §6).
1. **Verify containment, do not assume it** — a fully-removed ghost is now an unknown target, so a plain `@mention` of `@agent-<name>:<server_name>` is **silently ignored**: no reply, no A2A call, and — because the bridge drops an unmapped mention before any job is committed — no room notice either (fail-closed by absence, not by a visible refusal). Confirm containment by the _absence_ of a reply/delegation for that ghost in the bridge logs and delegation audit, not by expecting a message. The visible, actionable signals are: the command form `!ask <name>`/`/ask <name>` returns the unknown/unavailable-agent notice (issue #366 command copy); `!agents`/`/agents` no longer lists the agent in any sender's gallery and `!agents <name>` returns no cached entry; and any non-terminal durable ledger rows still queued for the removed target terminate fail-closed on the next atomic mapping re-read rather than dispatch. (A bounded `sender_policy_rejected` room notice appears only for a _bridged_ sender against a still-present mapping — not for the same-org local sender or a removed mapping.)
1. **Sweep the residual state in order** — the mapping removal stops delegation; retirement also clears what surrounds it: (a) have the appservice ghost **leave every room** it joined so users stop `@mention`ing a dead agent (drive it with the appservice token, content-free per-step timestamped evidence in the [`scripts/federation.sh`](../../../scripts/federation.sh) style); (b) handle the ghost's **Matrix account via MAS** — the exclusive `@agent-.*` appservice namespace means no one else can re-register the localpart, so document what deactivation adds beyond that (it prevents the bridge itself re-creating the ghost and marks the account closed) using the supported MAS admin actions in [docs/admin-console.md](../../../docs/admin-console.md), never a direct Synapse DB write; (c) **purge kagent sessions** for every `(room, ghost)` `contextId` via the session-purge mechanism (issue #100), because a stale session must not survive a re-used ghost name; (d) leave every delegation **audit record intact** — retention is a retirement invariant.
1. **State honestly what retirement does NOT do** — it does not erase Matrix history: prior messages and any federated copies of the room remain, and redaction is not erasure ([docs/federation.md](../../../docs/federation.md) §8.1, issue #137). It does not revoke a human user's access (that is IdP-driven offboarding, issue #153) and does not remove the operator console decision surface (issue #140). Retirement silences and cleans up **one agent principal**; it is not data deletion.

**Emergency disable (fast path).** To silence one misbehaving or compromised local agent immediately without full retirement: edit only its mapping — remove it, or narrow `allowedSenders` to empty — commit, and `flux reconcile kustomization bridge`. The bridge's next atomic reload refuses the agent before any A2A spend; there is no pod restart and no `kubectl edit` of a Flux-owned object. Emptying `allowedSenders` on a still-present mapping logs a `sender not allowed to invoke agent` bridge WARN for each attempt and posts nothing to the room for a local sender; full mapping removal drops the mention silently. Either way the containment evidence is the _absence_ of a reply and any A2A call (confirm in the bridge logs and delegation audit), plus `!agents` no longer offering the agent — not a visible refusal notice. Record the Git revision, the reconcile time, and that content-free per-step evidence. This is the same-organization sibling of the cross-org partner break-glass (issue #350); the day-2 handbook incident table carries the row ([docs/operations-handbook.md](../../../docs/operations-handbook.md) §5). Full room-leave and session purge follow as the deliberate retirement above once the incident is contained.
