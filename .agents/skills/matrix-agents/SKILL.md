...
## Runbook: author and ship an agent

This is the repository-owned composition loop. It reuses kagent Agent CRDs for execution, the governed MCP catalog for tools, agentgateway for model egress, and the deterministic demo model for regression checks. Do not add a second agent framework, evaluation service, model credential, or direct egress path.

1. **Scaffold once** — run `mise run agent:new <name>`. Review the generated `infra/kagent/agents/<name>/agent.yaml`, `apps/matrix-a2a-bridge/deploy/agents/<name>/kustomization.yaml`, and `evals/<name>/golden.json` before editing. Keep the mapping at `stage: dev`, the explicit full-MXID sender allowlist, the shared unprivileged ServiceAccount, and the three trace-content switches.
1. **Edit the narrow contract** — replace the placeholder description, system message, A2A skill, task prompt, and deterministic rubric. Reuse reviewed fragments from `infra/kagent/agent-zoo-prompts.yaml` and add only catalogued MCP tools through agentgateway. The Agent holds neither a provider key nor a Kubernetes API token.
1. **Run the one-Agent loop** — execute `mise run agent:test <name>` from the repository root. It selects only `evals/<name>/golden.json`, renders the effective Agent, calls the loopback-only deterministic backend, and applies the same verifier as CI's `mise run test:agents-golden`; it starts no cluster and makes no external request. A failed exact assertion prints an expected-versus-actual diff.
1. **Pin intentional contract changes** — when the runner reports a new effective contract SHA-256 after a reviewed prompt, skill, tool, model, deployment, or source-fragment edit, copy that exact digest into both `evals/<name>/golden.json` as `agent_contract_sha256` and the generated bridge mapping as `agentContractSHA256`. Never change a digest merely to silence an unexplained failure; review the effective source first, then rerun `agent:test`.
1. **Lint the composition** — run `mise run check:agents`; run `mise run check:agent-new` too when changing the scaffold itself. These gates prove the Agent/mapping/fixture bijection, pinned kagent schema, restricted runtime identity, sender policy, stage, MCP surface, and shared contract digest. Then run `mise run format`; the installed hooks or final `mise run agent:gate` own the complete warning-free repository gates.
1. **Open the review** — keep the Agent CRD, prompt fragments, mapping, and golden fixture in one PR so the effective contract cannot split across revisions. Record any staging-room or tool-boundary evidence separately; the offline golden pass predicts CI for the same fixture but does not prove live A2A, Matrix membership, provider quality, or production authorization.
1. **Promote deliberately** — after CI and staging-room acceptance, open a focused PR that changes only the mapping from `stage: dev` to `stage: prod`. Let Flux reconcile it; do not apply the CRD or mapping manually. Verify a fresh delegation and retain its content-free `agent_version` evidence.
1. **Roll back the whole contract** — revert the bad Agent CRD or prompt together with its fixture digest and mapping digest, or revert only the mapping for a mapping-only change. Let Flux reconcile `kagent` and `bridge`, then collect a fresh single-target attribution bundle. The restored `agent_version` must equal the pre-change value; a pod restart, Git message, or green render alone is not rollback proof.
