...
1. **Reply quality**: `fgentic_agent_reply_quality_signals_total{ghost,signal}` counts normalized human 👍/👎 reactions on process-known terminal agent replies. Room/reaction/sender/reply IDs stay on the content-free `fgentic.agent_reply.quality_signal` span, never Prometheus labels. This is best-effort feedback, not a durable or deduplicated success score.
...
1. **Content boundary:** every managed Python Agent must keep the exact three literal `false` controls documented in §9.2. Bridge failures use fixed-name, attribute-free events; agentgateway adds no prompt/completion expressions; and the Collector is a forwarder, not a redactor. Content-free traces remain linkable through Matrix and agent identifiers, so restrict Jaeger/Grafana access and retention.
...
1. **Validation boundary:** `mise --cd apps/matrix-a2a-bridge run test` proves bridge span lifecycle, A2A-error redaction, and exact outbound propagation. `mise run test:kagent-trace-privacy` proves the pinned kagent runtime suppresses prompt, completion, tool-argument, and tool-result sentinels in the final trace representation while retaining bounded GenAI metadata. `mise run test:tracing` proves only the Collector → Jaeger → Grafana datasource path with a synthetic span. None proves cross-component continuity; issue #35 requires one installed-cluster mention whose bridge, gateway, agent, and model spans share a trace ID.
