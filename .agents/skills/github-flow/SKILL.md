...
1. Immediately before merging, fetch `origin/main` and verify the PR is mergeable and clean, its head still equals the reviewed green SHA, every required check succeeded, and no review finding remains unresolved. If the base requires a rebase, rebase, push, and repeat both gates. Guard the squash merge against a last-second head change with `gh pr merge <n> --squash --delete-branch --match-head-commit <reviewed-sha>`, then `git checkout main && git pull`.
