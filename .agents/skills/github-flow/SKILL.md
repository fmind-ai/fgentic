...
1. Labels: `agent-ready` = groomed, pick up as-is · `needs-human` = blocked on a maintainer decision/account/approval/spend — do the preparable parts, then hand off explicitly · `area/*` (bridge, infra, identity, matrix, models, federation, observability, security, ci, docs, community) · `kind/*` (feature, fix, test, chore, docs, research, epic) · `by/*` (red, blue, green, hosted) = provenance of a PR's parallel session lane ([docs/hosted-agents.md](../../../docs/hosted-agents.md)), never a pickup gate.
...
1. `gh pr create --title "..." --body-file <path>` then watch CI with `gh pr checks <n> --watch`. Use `gh pr view <n> --json headRefOid,statusCheckRollup` to require every reported check on that head to be terminal and successful, or to inspect and justify an intentionally neutral/skipped conclusion. This repository has no ruleset-required status-check set, so `gh pr checks --required` returning no checks is never merge evidence. In a non-interactive shell `gh` may refuse with "prefer_editor_prompt not supported in non-tty mode" — run `gh config set prefer_editor_prompt disabled` once.
1. Obtain independent peer coding-agent review of that same `headRefOid` and address every actionable finding. A commit, rebase, or conflict resolution changes the head and invalidates both CI and review; push the new head and repeat both gates.
1. Immediately before merging, fetch `origin/main` and verify the PR is mergeable and clean, its head still equals the reviewed green SHA, the full hosted check rollup remains acceptable, and no review finding remains unresolved. If the base requires a rebase, rebase, push, and repeat both gates. Guard the squash merge against a last-second head change with `gh pr merge <n> --squash --delete-branch --match-head-commit <reviewed-sha>`, then `git checkout main && git pull`.
