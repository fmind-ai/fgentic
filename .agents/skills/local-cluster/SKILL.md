...
1. After a recreate, redo the out-of-band steps: Gateway API CRDs, the `sops-age` Secret, `scripts/local-ca.sh`, `scripts/local-adc.sh`, then `flux bootstrap github … --path=clusters/local` (order and commands in the matrix-agents runbook).
...
1. **LLM calls fail locally**: Vertex AI auth is the ADC Secret (`gcp-adc` in `agentgateway-system`) — expired/missing ADC is the usual cause; rerun `scripts/local-adc.sh`. The `gcp_project` in platform-settings is real even locally.
