...
1. **Claim:** before editing, skip issues with an active assignee or `status/in-progress` lease. Assign yourself, add `status/in-progress`, and leave a UTC-stamped owner/branch comment. Hosted agents without GitHub write access must receive an already-claimed issue. See the [github-flow skill](.agents/skills/github-flow/SKILL.md) for stale-claim and handoff rules.
1. **Validate:** use focused checks during development. Near PR readiness, `mise run agent:gate` serializes the required warning-free `mise run check` then `mise run test` across worktrees on the same host. Do not repeatedly run the aggregate gates, and never weaken an assertion, add a skip, or suppress a lint error to get green.
